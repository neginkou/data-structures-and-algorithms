...
const hasChildrenValues = (arr, character) => {
  const characterObject = arr.find((char) => char.name === character);

  if (characterObject) {
    const valuesArray = Object.values(characterObject);
    const childrenPropertyIndex = valuesArray.findIndex((value) => Array.isArray(value) && value.length > 0);

    return childrenPropertyIndex !== -1;
  }
