...
function transformToLis(obj){
  ...
  const resultList = Object.entries(obj).map(([key, value]) => `<li>${key}: ${value}</li>`);
...
const count = (target, input) => {
  const totalCount = input.reduce((acc, currentArray) => {
    const arrayCount = currentArray.reduce((arrayAcc, element) => (element === target ? arrayAcc + 1 : arrayAcc), 0);
    return acc + arrayCount;
  }, 0);
...
const totalSum = (input) => {
  const sum = input.reduce((acc, currentArray) => {
    const arraySum = currentArray.reduce((arrayAcc, element) => arrayAcc + element, 0);
    return acc + arraySum;
  }, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  const result = input.map((currentArray) => {
    const filteredArray = currentArray.filter((element) => typeof element === 'number' && element % 5 === 0);
    const poweredArray = filteredArray.map((number) => Math.pow(2, number));
